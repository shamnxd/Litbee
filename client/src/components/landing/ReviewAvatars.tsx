import React from 'react';

export const ReviewAvatars: React.FC = () => {
    const users = [
        {
            name: "Richard Nelson",
            username: "@richard",
            image: "https://images.unsplash.com/photo-1633332755192-727a05c4013d?q=80&w=200",
        },
        {
            name: "Avery Johnson",
            username: "@averywrites",
            image: "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?q=80&w=200",
        },
        {
            name: "Jordan Lee",
            username: "@jordantalks",
            image: "https://raw.githubusercontent.com/prebuiltui/prebuiltui/main/assets/userImage/userImage1.png",
        },
        {
            name: "Noah Patel",
            username: "@noahpatel",
            image: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=200&h=200&auto=format&fit=crop",
        },
        {
            name: "Oliver Brooks",
            username: "@oliverbrooks",
            image: "https://randomuser.me/api/portraits/men/75.jpg",
        },
    ];

    return (
        <div className="flex -space-x-4 pr-3">
            {users.map((user, index) => (
                <div key={index} className="group relative">
                    <div className="absolute border border-gray-200 pointer-events-none group-hover:pointer-events-auto opacity-0 group-hover:opacity-100 -top-16 group-hover:mr-2 left-1/2 -translate-x-1/2 transition-all duration-400 pl-4 pr-6 py-2 rounded text-nowrap bg-white shadow-xl z-50">
                        <div className="flex flex-col">
                            <div className="flex items-center gap-1">
                                <p className="font-medium text-sm text-gray-900">{user.name}</p>
                                <svg className="mt-0.5" width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path fillRule="evenodd" clip-rule="evenodd" d="M4.555.72a4 4 0 0 1-.297.24c-.179.12-.38.202-.59.244a4 4 0 0 1-.38.041c-.48.039-.721.058-.922.129a1.63 1.63 0 0 0-.992.992c-.071.2-.09.441-.129.922a4 4 0 0 1-.041.38 1.6 1.6 0 0 1-.245.59 3 3 0 0 1-.239.297c-.313.368-.47.551-.56.743-.213.444-.213.96 0 1.404.09.192.247.375.56.743.125.146.187.219.24.297.12.179.202.38.244.59.018.093.026.189.041.38.039.48.058.721.129.922.163.464.528.829.992.992.2.071.441.09.922.129.191.015.287.023.38.041.21.042.411.125.59.245.078.052.151.114.297.239.368.313.551.47.743.56.444.213.96.213 1.404 0 .192-.09.375-.247.743-.56.146-.125.219-.187.297-.24.179-.12.38-.202.59-.244a4 4 0 0 1 .38-.041c.48-.039.721-.058.922-.129.464-.163.829-.528.992-.992.071-.2.09-.441.129-.922a4 4 0 0 1 .041-.38c.042-.21.125-.411.245-.59.052-.078.114-.151.239-.297.313-.368.47-.551.56-.743.213-.444.213-.96 0-1.404-.09-.192-.247-.375-.56-.743a4 4 0 0 1-.24-.297 1.6 1.6 0 0 1-.244-.59 3 3 0 0 1-.041-.38c-.039-.48-.058-.721-.129-.922a1.63 1.63 0 0 0-.992-.992c-.2-.071-.441-.09-.922-.129a4 4 0 0 1-.38-.041 1.6 1.6 0 0 1-.59-.245A3 3 0 0 1 7.445.72C7.077.407 6.894.25 6.702.16a1.63 1.63 0 0 0-1.404 0c-.192.09-.375.247-.743.56m4.07 3.998a.488.488 0 0 0-.691-.69l-2.91 2.91-.958-.957a.488.488 0 0 0-.69.69l1.302 1.302c.19.191.5.191.69 0z" fill="#2196F3" />
                                </svg>
                            </div>
                            <span className="text-[10px] text-slate-500 uppercase tracking-tight">{user.username}</span>
                        </div>
                        <div className="size-2 border-r border-b border-gray-200 bg-white rotate-45 absolute left-1/2 -translate-x-1/2 -bottom-[5px]"></div>
                    </div>
                    <img
                        src={user.image}
                        alt={user.name}
                        className="size-10 rounded-full border-2 border-[#F5F5F5] group-hover:-translate-y-1 transition-all duration-300 cursor-pointer object-cover shadow-sm"
                    />
                </div>
            ))}
        </div>
    );
};
