...
export class UrlsService implements IUrlsService {
  ...
  async updateUrl(
    ...
    const updatedUrl = await this.urlsRepository.findOneAndUpdate(
      { _id: id, userId },
      { $set: updateData },
    );
  ...
  async deleteUrl(id: string, userId: string): Promise<{ message: string }> {
    const isDeleted = await this.urlsRepository.deleteOne({ _id: id, userId });
    if (!isDeleted) {
      throw new NotFoundException(URL_MESSAGES.ERRORS.NOT_FOUND);
    }
  ...
  async findAllByUser(
    ...
    const [urls, total] = await Promise.all([
      this.urlsRepository.find(query, skip, limit, { createdAt: -1 }),
      this.urlsRepository.countDocuments(query),
    ]);
  ...
  async findByCode(shortCode: string): Promise<UrlResponse> {
    const url = await this.urlsRepository.findOne({ shortCode });
    if (!url) {
  ...
  async checkSlugAvailability(
    ...
    const existing = await this.urlsRepository.findOne(query);
    return !existing;
  }
}
