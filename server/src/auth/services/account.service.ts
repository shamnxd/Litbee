...
export class AccountService implements IAccountService {
  constructor(
    ...
    @Inject(I_REDIS_SERVICE) private readonly redisService: IRedisService,
    @Inject(I_MAIL_SERVICE) private readonly mailService: IMailService,
  ...
  async sendVerificationEmail(email: string): Promise<{ message: string }> {
    ...
    const expiry = this.configService.get<number>('OTP_EXPIRES_IN_SEC') || 300;
    await this.redisService.set(`otp:${email}`, otp, Number(expiry));
  ...
  async verifyEmail(email: string, otp: string): Promise<AuthResponse & { message: string }> {
    ...
    const updatedUser = await this.authRepository.findByIdAndUpdate(user._id.toString(), { $set: { isVerified: true } });
    if (updatedUser) user = updatedUser;
    await this.redisService.del(`otp:${email}`);

    const response = await this.identityService.issueTokens(user);
    return {
  ...
  async forgotPassword(email: string): Promise<{ message: string }> {
    ...
    const expiry = this.configService.get<number>('RESET_TOKEN_EXPIRES_IN_SEC') || 3600;
    await this.redisService.set(`reset:${resetToken}`, email, Number(expiry));
  ...
  async resetPassword(token: string, newPass: string): Promise<{ message: string }> {
    ...
    await this.authRepository.findByIdAndUpdate(user._id.toString(), { $set: { password: hashedPassword } });
